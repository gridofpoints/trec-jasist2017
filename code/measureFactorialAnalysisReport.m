%% measureFactorialAnalysisReport
% 
% Prints the summary report of the factorial analyses across measures on 
% the requested  experimental collections.

%% Synopsis
%
%   [] = measureFactorialAnalysisReport(tag, varargin)
%  
%
% *Parameters*
%
...
function [] = measureFactorialAnalysisReport(tag, varargin)
    ...
    for t = 1:tracks
        ...
        for lug = 1:EXPERIMENT.taxonomy.lug.number
            ...
            try
                ...
                serload(EXPERIMENT.pattern.file.anova(trackID, tag, lugType, measureID), anovaTableID, anovaSoAID);
                
                eval(sprintf('anovaTable.(''%1$s'').(''%2$s'') = %3$s;', trackID, lugType, anovaTableID));
                eval(sprintf('soa.(''%1$s'').(''%2$s'') = %3$s;', trackID, lugType, anovaSoAID));
                ...
                eval(sprintf('anovaTable.(''%1$s'').(''%2$s'') = [];', trackID, lugType));
                eval(sprintf('soa.(''%1$s'').(''%2$s'') = [];', trackID, lugType));
            end;
